...
def init_llms():
    ...
    market_llm = ChatGroq(model="llama-3.3-70b-versatile", temperature=0.2, api_key=os.environ.get("GROQ_API_KEY",""))
    return gpt, groq_llm, market_llm
...
def market_agent(idea):
    _,_,m = init_llms()
    return m.invoke([SystemMessage(content="You are a market analyst. Provide: Market Sizing (TAM/SAM/SOM), Competitive Landscape (3-5 competitors), Key Market Trends, Go-to-Market Recommendation."), HumanMessage(content=f"Product: {idea}")]).content

def stories_agent(idea, prd=""):
    _,g,_ = init_llms()
    return g.invoke([SystemMessage(content="You are an Agile PM. Generate 8-12 user stories grouped in Epics. Format each as: ### Story N: Title\n**As a** user **I want** goal **so that** benefit.\n**Acceptance Criteria:**\n- [ ] item\n**Story Points:** N\n**Priority:** High/Medium/Low"), HumanMessage(content=f"Product: {idea}\nPRD: {prd[:1000]}")]).content

def risk_agent(idea, market=""):
    _,g,_ = init_llms()
    return g.invoke([SystemMessage(content="You are a risk consultant. Generate tables for Technical Risks, Market Risks, Execution Risks with columns: Risk|Severity|Likelihood|Mitigation. Then list Top 3 Priority Risks."), HumanMessage(content=f"Product: {idea}\nMarket: {market[:800]}")]).content

def route(q, idea, outputs, history):
    gpt,groq,market = init_llms()
    ql = q.lower()
    if any(k in ql for k in ["story","epic","backlog","acceptance"]): llm,a,b = groq,"Groq/Llama","badge-groq"; sys="You are an Agile PM assistant."
    elif any(k in ql for k in ["market","competitor","pricing","tam","gtm"]): llm,a,b = market,"Groq/Llama","badge-groq"; sys="You are a market analyst."
    elif any(k in ql for k in ["risk","threat","mitigation"]): llm,a,b = groq,"Groq/Llama","badge-groq"; sys="You are a risk consultant."
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("🟦 **GPT-4o** — PRD\n\n🟪 **Groq/Llama** — Market, Stories, Risk")
    st.divider()
    ok = st.text_input("OpenAI Key", value=os.environ.get("OPENAI_API_KEY",""), type="password")
    gk = st.text_input("Groq Key (free)", value=os.environ.get("GROQ_API_KEY",""), type="password")
...
with tabs[0]:
    if not st.session_state.done:
        ...
        if st.button("🚀 Generate PM Suite", type="primary"):
            if not idea.strip(): st.warning("Enter a product idea first.")
            elif not all([os.environ.get("OPENAI_API_KEY"), os.environ.get("GROQ_API_KEY")]): st.error("Add both API keys in the sidebar.")
            else:
                ...
                with st.status("Orchestrating agents...", expanded=True) as s:
                    ...
                    st.write("🟪 Groq → Market..."); st.session_state.outputs["market"] = market_agent(idea); st.write("✅ Market done")
...
for tab,badge,label,title,key,fn,dl in [
    ...
    (tabs[2],"Groq","Generated by Groq/Llama","📊 Market Analysis","market",lambda:market_agent(st.session_state.idea),"Market.md"),
    (tabs[3],"Groq","Generated by Groq/Llama","🗂 User Stories","stories",lambda:stories_agent(st.session_state.idea,st.session_state.outputs.get("prd","")),"Stories.md"),
    (tabs[4],"Groq","Generated by Groq/Llama","⚠️ Risk Assessment","risks",lambda:risk_agent(st.session_state.idea,st.session_state.outputs.get("market","")),"Risks.md"),
]:
    with tab:
